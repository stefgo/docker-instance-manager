 ...
 *
 * Two kinds of value are settled here rather than in the schema. `DIM_CLIENT_PORT` wins over
 * the file, so a container can move the agent's port without a mounted config file. And the
 * two tolerated settings -- `logLevel` and `allowSelfSignedCertificates` -- lose a wrong
 * value with a warning here, so the schema behind this can stay strict about everything it
 * does see.
 */
function prepare(raw: unknown): Record<string, unknown> {
    ...
    const port = process.env.DIM_CLIENT_PORT?.trim();
    ...
    if (
        ...
        logger.warn("Ignoring allowSelfSignedCertificates in config.yaml: expected true or false");
    ...
    if (out.registrationSecret !== undefined && out.registrationSecret !== null) {
        logger.warn(
            "Ignoring registrationSecret in config.yaml: it is no longer read. Register with the " +
                "setup PIN from this log, or set DIM_REGISTRATION_SECRET instead.",
        );
    }
...
function loadConfig(): ClientConfig {
    ...
    if (!parsed.success) {
        logger.fatal({ path: CONFIG_PATH }, `Invalid config.yaml -- ${firstIssue(parsed.error)}`);
        process.exit(1);
    }
 ...
 * where nobody reads the agent's log. Environment only -- `DIM_REGISTRATION_SECRET`, or
 * `DIM_REGISTRATION_SECRET_FILE` naming a file that holds it (a Docker or Podman secret).
 *
...
function readRegistrationSecret(): string | null {
    const value = process.env.DIM_REGISTRATION_SECRET?.trim() || null;
    const file = process.env.DIM_REGISTRATION_SECRET_FILE?.trim() || null;
    if (value && file) {
        logger.fatal("Set DIM_REGISTRATION_SECRET or DIM_REGISTRATION_SECRET_FILE, not both");
        process.exit(1);
    }
    ...
    try {
        ...
        const code = (err as NodeJS.ErrnoException).code;
        logger.fatal(`DIM_REGISTRATION_SECRET_FILE cannot be read at ${file}${code ? ` (${code})` : ""}`);
        process.exit(1);
    }
    if (!content) {
        logger.fatal(`DIM_REGISTRATION_SECRET_FILE is empty: ${file}`);
        process.exit(1);
    }
    return content;
}
