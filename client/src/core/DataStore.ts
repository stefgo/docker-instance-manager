...
/**
 * Where the agent keeps what it has to survive a restart -- the policy the server sent, the
 * schedule state, and the activity events nobody has acknowledged yet.
 *
 ...
 * next recreate -- which is every self-update. This directory is a named volume
 * (`compose.yaml`), and `DIM_CLIENT_DATA_DIR` moves it for an agent that runs outside one.
 */
export const DATA_DIR =
    process.env.DIM_CLIENT_DATA_DIR?.trim() || path.resolve(ROOT_DIR, "data");
