 ...
 * and whoever registers the agent gets its Docker socket, which is root on the host. Before
 * this PIN, anyone who could reach the agent's web port could do that through the register
 * page, and outbound registration needed a secret the operator had to write into config.yaml
 * first.
 *
 * So the check is a shared secret that is printed to the agent's log, where only someone who
 * can already read the machine's logs (`docker logs dim-client`) can see it. An agent given
 * `DIM_REGISTRATION_SECRET` for an unattended rollout accepts that on `/ws/register` as well.
 *
