...
| `DockerController`      | Docker state retrieval, action dispatch to agents, image update checks. Records `action.requested` under the action's id, and `action.unconfirmed` when no answer comes back (connection lost, timeout). The outcome itself is the agent's; the server records `action.failed` only for an agent too old to report it. |
...
- An **action from the dashboard** is sent with an `actionId` the server generates. The server records `action.requested` under it, and the agent stamps the same id on every container and image event the action goes on to cause. The agent knows which containers it is about to touch *before* it touches them, because it is the side doing the work. It also reports the outcome under that id (`action.completed` / `action.failed`, with the error), through its queue: a connection that closes mid-action loses the `DOCKER_ACTION_RESULT`, but not the outcome. The server then records only `action.unconfirmed`, which the dashboard drops as soon as the agent's outcome arrives.
