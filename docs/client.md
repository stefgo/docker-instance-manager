...
- **Image update** is two steps, each usable on its own: `updateImage(target)` only pulls and reports which image the tag pointed to before and after; `updateContainer(id)` recreates one container. `pullAndRecreate(target, containerIds?, force?)` puts them together for `image:update`: it pulls, then recreates every container configured with the reference that does not run the image the tag now points to — limited to `containerIds` where the request names them. A container already on the new image is left alone, unless `force` is set. The auto-update calls the two steps itself, because it decides per container.
...
- **Correlation scopes.** `executeAction` opens a scope keyed on the server's `actionId` and tells it which containers the work is about to touch, *before* it touches them — `updateImage` adds each affected container as it resolves the list. Every event about a covered subject is stamped with that id on its way past. This is the side doing the work, so nothing is matched by name and nothing depends on a time window. A scope closes as soon as it has seen the events it said to expect; a 15-second grace window is only the fallback for one that never comes. When the action has returned, `executeAction` reports its outcome under the same id (`action.completed`, or `action.failed` with the error) and marks the `DOCKER_ACTION_RESULT` `reported: true`, so the server does not record it twice. The event goes through the queue, so the outcome of an action whose connection closed midway still reaches the server.
- **The first health status after a start.** A healthcheck reports after its first probe, often long after the scope has closed, and holding the scope open for it would stamp everything else that happens to the container meanwhile. So a start, restart or recreate reads the container's effective healthcheck *before* starting it and registers a wait by container id; the next `container.health` for that id carries the operation's id, whenever it comes. The id is exact: a recreate gives a new one, and a start resets the status to `starting`. No timer ends the wait — every state snapshot the watcher reads does, once the container is gone, no longer running or past `starting`, which also clears a status lost in a broken event stream (a reconnect reads a snapshot for that reason).
