...
- With `enableRegisterPage: false` neither the page nor `POST /api/register` exists. A PIN is then generated only while the agent is unregistered, has no `serverUrl` and no `DIM_REGISTRATION_SECRET`, and it is discarded once the server has registered the agent.
