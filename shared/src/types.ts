...
// ── Auto-update, as the server observes it ───────────────────────────────────

/**
 * The last run of one schedule on one host, read back out of the `autoupdate.run` event the
 * agent reported. Nothing is stored twice for this: the events are the record, so the
 * overview survives a server restart -- which the module-level `lastRun` of the deleted
 * server-side scheduler did not.
 */
export interface AutoUpdateRunSummary {
    /** The schedule the run belongs to: `host`, or `project:<name>`. */
    schedule: string;
    occurredAt: string;
    level: ActivityLevel;
    ...
    failed: number | null;
    skipped: number | null;
    /** A run that was owed while the host was off, made up after the start. */
    catchUp: boolean;
    /** A run somebody asked for, rather than one a schedule reached. */
    manual: boolean;
}

/** One host's autonomous auto-update in the fleet overview. */
export interface AutoUpdateAgentStatus {
    clientId: string;
    clientName: string;
    online: boolean;
    version: string | null;
...
export interface AutoUpdateStatusResponse {
    agents: AutoUpdateAgentStatus[];
    /** The fleet-wide default schedule, for the hosts that inherit it. */
    defaultCron: string;
}
